...
import {
  EvModeElementStoreEventChangeTypeEnum,
  type EvModeElementStoreEvent,
  type IAppStore,
...
import {
  ElementsStore,
  IElementsStore,
  IElementsStoreRecord,
} from "./ElementsStore";
import LiteEvent from "./LiteEvent";

type InternalStoreType = Map<EvModElementTypeEnum, IElementsStore>;

const getEmptyStore = (): InternalStoreType =>
  new Map(
    Object.keys(EvModElementTypeEnum).map((elementType) => [
      elementType as EvModElementTypeEnum,
      new ElementsStore(),
    ])
  );
...
export default class InMemoryStore implements IAppStore {
  ...
  ): IElementsStoreRecord | undefined {
  ...
  private internalStore: InternalStoreType = new Map<
  ...
  private async commit() {
    return miro.board.setAppData(
      APP_STORE,
      JSON.stringify(Array.from(this.internalStore.entries()))
    );
  ...
  async sync(): Promise<void> {
    ...
    const entries: Array<[string, { store: Array<any> }]> =
      JSON.parse(serializedAppData);
  ...
  get onChange() {
    return this.onChangeHandlers.expose();
  }
  ...
  async addElementPayload(
    elementType: EvModElementTypeEnum,
    miroElementId: string,
    payload: any
  ): Promise<void> {
    const store = this.internalStore.get(elementType);
    if (!store || !store.getElementName(miroElementId))
  ...
  async rename(
    ...
    console.log(
      `renaming ${oldName} to ${newName} in element type: ${elementType}`
    );
