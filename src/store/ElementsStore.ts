...
export interface IElementsStore {
  ...
  rename(elementName: string, newName: string): void;
}
...
export class ElementsStore implements IElementsStore {
  ...
  getById(miroElementId: string): IElementsStoreRecord | undefined {
    const result = this.list().filter(
      (el) => el.miroElementId === miroElementId
    );
    return result.length ? result[0] : undefined;
  }
  listCopies(miroElementId: string): IElementsStoreRecord[] {
    return this.list().filter(
      (el) => el.originalMiroElementId === miroElementId
    );
  }
  ...
  deleteElement(miroElementId: string): void {
    const index = this.store.findIndex(
      (record) => miroElementId === record.miroElementId
    );
    if (index === -1) return;
    this.store.splice(index, 1);
  }
  getElementName(miroElementId: string): string | undefined {
    const result = this.store.find(
      (record) => record.miroElementId === miroElementId
    );
  ...
  getMiroElementId(elementName: string): string | undefined {
    const result = this.store.find(
      (record) =>
        record.elementName === elementName && !record.originalMiroElementId
    );
    return result?.miroElementId;
  }

  rename(elementName: string, newName: string): void {
    this.store = this.store.map((content) =>
      content.elementName === elementName
        ? { ...content, elementName: newName }
        : content
    );
  }
}
