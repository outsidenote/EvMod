...
export default function AddElement() {
    ...
    const getFillColor = () => {
        switch (elementType) {
            case ElementTypeEnum.Event:
                return '#F5D22B'
            case ElementTypeEnum.Command:
                return '#2D9BF0'
            case ElementTypeEnum.ReadModel:
                return '#8FD14F'
    ...
    const getColor = () => {
        switch (elementType) {
            ...
            case ElementTypeEnum.ReadModel:
                return '#1A1A1A'
            case ElementTypeEnum.Processor:
            case ElementTypeEnum.Screen:
                return '#000000'
            default:
                return '#000000'
    ...
    const createShape = async () => {
        if (!!elementType && [
            ...
            ElementTypeEnum.ReadModel,
            ElementTypeEnum.Screen].includes(elementType))
        ...
        else if (elementType == ElementTypeEnum.Processor) {
        ...
        else if (elementType == ElementTypeEnum.Swimlane) {
    ...
    const createElementMetadata = async (element: Shape | Frame | Group) => {
        if (!elementType)
        ...
        if (!!element && !(element.type === "group"))
            await element.setMetadata(ELEMENT_METADTA_KEY, { ...metadata });
    }

    const updateEventsCatalog = async (element: Shape | Frame | Group) => {
        setEventsCatalog(eventsCatalog.set(elementName, element.id))
        await miro.board.setAppData(EVENTS_CATALOG_KEY, eventsCatalog.toObject());
    }

    const handleSubmit = async () => {
        const shape = await createShape();
        if(!shape)
            throw new Error('shape was not created');
        await createElementMetadata(shape);

        if(elementType == ElementTypeEnum.Event)
            updateEventsCatalog(shape);
    ...
    return (
        <div>
            ...
            <Stack spacing={2} direction="column">
                <Button variant="contained" color="warning" onClick={() => handleOpen(ElementTypeEnum.Event)}>{ElementTypeEnum.Event}</Button>
                <Button variant="contained" color="info" onClick={() => handleOpen(ElementTypeEnum.Command)}>{ElementTypeEnum.Command}</Button>
                <Button variant="contained" color="success" onClick={() => handleOpen(ElementTypeEnum.ReadModel)}>{ElementTypeEnum.ReadModel}</Button>
                <Button startIcon={<CogsIcon />} variant="outlined" color="info" onClick={() => handleOpen(ElementTypeEnum.Processor)}>{ElementTypeEnum.Processor}</Button>
                <Button startIcon={<ScreenIcon />} variant="outlined" color="info" onClick={() => handleOpen(ElementTypeEnum.Screen)}>{ElementTypeEnum.Screen}</Button>
                <Button startIcon={<Crop169Icon />} variant="outlined" color="info" onClick={() => handleOpen(ElementTypeEnum.Swimlane)}>{ElementTypeEnum.Swimlane}</Button>
            </Stack>
            <Modal
                open={open}
                onClose={handleClose}
