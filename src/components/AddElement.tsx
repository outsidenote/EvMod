...
export default function AddElement() {
  ...
  const [elementType, setElementType] = React.useState<
    EvModElementTypeEnum | undefined
  >();
  const [elementName, setElementName] = React.useState("");
  const [store] = React.useContext(Context);

  const [elementNames, setElementNames] = React.useState<string[]>([]);
  ...
  const getWidth = () => {
    if (elementType == "Screen") return 300;
    return 200;
  };
  ...
  const addElementRecord = async (
    element: Shape | Frame | Image
  ): Promise<string | undefined> => {
    if (!elementType) throw new Error("No Element Type");
    console.log(
      "AddElement: addElementRecord: elementType:",
      elementType,
      "element id:",
      element.id,
      "element name:",
  ...
  const linkShapeTooriginal = async (
    shape: MiroElementType,
    ...
    if (
      ...
      return;
    const { id: boardId }: BoardInfo = await miro.board.getInfo();
    console.log("boardId:", boardId);
    (
      shape as Shape
    ).linkedTo = `https://miro.com/app/board/${boardId}/?moveToWidget=${miroElementId}`;
    await shape.sync();
  };

  const addElementMetadata = async (
    ...
    const metadata: IElementMetadata = {
      ...
      elementName,
      copyOf:
        originalMiroElementId !== shape.id ? originalMiroElementId : undefined,
    };
    console.log("AddElement: addE;e,emtMetadata: metadata:", metadata);
    await (shape as Shape | Image).setMetadata(
  ...
  const handleSubmit = async () => {
    ...
    console.log(
      "AddElement: handleSubmit: shapeId:",
      shape.id,
      "originalId:",
      originalMiroElementId
    );
    if (originalMiroElementId) {
      await addElementMetadata(originalMiroElementId, shape);
      if (originalMiroElementId !== shape.id) {
        await linkShapeTooriginal(shape, originalMiroElementId);
      }
    }
    ...
    <div>
      ...
      <Stack spacing={2} direction="column">
        <Button
          variant="contained"
          color="warning"
          onClick={() => handleOpen(EvModElementTypeEnum.Event)}
        >
          {EvModElementTypeEnum.Event}
        </Button>
        <Button
          variant="contained"
          color="info"
          onClick={() => handleOpen(EvModElementTypeEnum.Command)}
        >
          {EvModElementTypeEnum.Command}
        </Button>
        <Button
          variant="contained"
          color="success"
          onClick={() => handleOpen(EvModElementTypeEnum.ReadModel)}
        >
          {EvModElementTypeEnum.ReadModel}
        </Button>
        <Button
          ...
          variant="outlined"
          color="info"
          onClick={() => handleOpen(EvModElementTypeEnum.Processor)}
        >
          {EvModElementTypeEnum.Processor}
        </Button>
        <Button
          startIcon={<ScreenIcon />}
          variant="outlined"
          color="info"
          onClick={() => handleOpen(EvModElementTypeEnum.Screen)}
        >
          {EvModElementTypeEnum.Screen}
        </Button>
        {/* Bnaya asked to remove this option  */}
