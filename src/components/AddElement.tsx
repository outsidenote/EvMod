...
import { EvModElementTypeEnum, MiroElementType } from '../types/element.types';
...
export default function AddElement() {
    ...
    const linkShapeTooriginal = async (shape: MiroElementType, miroElementId: string) => {
        ...
        (shape as Shape).linkedTo = `https://miro.com/app/board/${boardId}/?moveToWidget=${miroElementId}`;
        await shape.sync();
    }
    ...
    const handleSubmit = async () => {
        ...
        const miroElementId = await addElementRecord(shape);
        console.log('AddElement: handleSubmit: shapeId:', shape.id, 'originalId:', miroElementId)
        if (miroElementId && miroElementId !== shape.id) {
            await linkShapeTooriginal(shape, miroElementId);
        }
        handleClose();
    }
